...
import BusinessConditions from './business'
...
export default class KafkaToSql {
  constructor (coreServices, config) {
    ...
    this.business = new BusinessConditions(coreServices, this.pluginInfos, config)
  ...
  worker = async (message) => {
    ...
    if (this.kafkaKeys.some((element) => element === message.key)) {
      ...
      await this.business.processBusinessConditions(messageValue?.content, message.key)
    } else {
