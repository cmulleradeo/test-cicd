...
import ProductDevelopmentBusiness from './business'
...
export default class KafkaToSql {
  constructor (coreServices, config) {
    ...
    this.business = new ProductDevelopmentBusiness(coreServices, this.pluginInfos, config)
  ...
  worker = async (message) => {
    ...
    if (this.kafkaKeys.some((element) => element === message.key)) {
      ...
      await this.business.processProductDevelopment(messageValue?.content, message.key)
    } else {
